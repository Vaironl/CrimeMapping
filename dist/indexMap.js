...
var _Crime = _interopRequireDefault(require("../Classes/Crime"));
...
function getDataSet(crimes) {
  for (var i = 0; i < crimes.length; i++) {
    ...
    var crime = JSON.parse(crimes[i]); // 2.) Store the properties

    crimeDataSet[i] = new _Crime.default(crime._id, crime.crimeCat, crime.date, crime.time, crime.offenses, crime.severity, crime.lat, crime.lng);
  }
}
...
function getPoints(crimes) {
  ...
  for (var i = 0; i < crimes.length; i++) {
    ...
    */
    ...
    googlePoints[i] = new google.maps.LatLng(crimeDataSet[i].lat(), crimeDataSet[i].lng()); // !!! Still need to do something with the weight property !!!
  }
